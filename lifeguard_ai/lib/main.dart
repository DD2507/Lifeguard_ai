...
class _PatientDetailsScreenState
  ...
  Future<void> loadPatient() async {
    ...
    try {
      ...
      final patientRoomId = data["room"]?.toString();
      ...
      for (final roomData in rooms) {
        ...
        if (roomData is Map) {
          ...
          if (roomId == patientRoomId) {

            matchingRoom = Map<String, dynamic>.from(roomData);
      ...
      setState(() {
        ...
        liveRoom = matchingRoom;

        loading = false;

        error = "";

        if (!controllingFan && !controllingBuzzer) {

          final action = data["recommendedAction"];

          if (action is Map) {

            fanStatus = action["fan"] == true;

            buzzerStatus = action["buzzer"] == true;
      ...
      debugPrint("Patient room: $patientRoomId");

      debugPrint("Live room data: $matchingRoom");
